class ProgrammingCodeExtension extends Extension {
    init() {
        ...
        this.createCodeArea();
        this.createControls();
        this.stepCounterEl = this.createElement('div', { 
            className: 'step-counter' 
        }, ['Pasos: ', this.createElement('span', { id: 'step-count' }, ['0'])]);
        this.tabContent.appendChild(this.stepCounterEl);
...
`
    ...
    createControls() {
        ...
        const directBtn = this.createElement('button', {
            className: 'btn btn-secondary',
            textContent: 'Directo',
            onclick: () => this.runImmediate()
        });
    ...
    runImmediate() {
        const code = this.codeArea.value;
        const stepCount = this.app.getExtension('interpreter').runImmediate(code);
...
window.__extensions.push({
    ...
    constructor: ProgrammingCodeExtension
});
