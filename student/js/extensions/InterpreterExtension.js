class InterpreterExtension extends Extension {
    ...
    step() {
		...
		if (!this.immediateMode && this.state.get('hero.moving')) {
            return true; 
        }
    ...
    runImmediate(code) {
        ...
        while (this.state.get('program.running') && 
               this.stepCounter < Config.MAX_PROGRAM_STEPS) {
            if (!this.step()) break;
        }
    ...
    updateDebugInfo() {
        ...
        const stepEl = document.getElementById('debug-steps');
        if (stepEl) stepEl.textContent = this.stepCounter;
    }
...
window.__extensions.push({
    name: 'interpreter',
    constructor: InterpreterExtension
});
